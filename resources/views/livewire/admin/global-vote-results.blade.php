<div>
    ...
    <div style="display:flex;align-items:flex-start;justify-content:space-between;gap:16px;margin-bottom:24px">
        <div>
            <h1 style="font-size:22px;font-weight:800;color:#241038;margin-bottom:4px">Vote Results</h1>
            <p style="color:#9ca3af;font-size:13.5px">Real-time tally across all your events.</p>
        </div>
        <select wire:model.live="selectedEventId"
                style="border:1.5px solid #e5e7eb;border-radius:10px;padding:9px 16px;font-size:13.5px;color:#241038;outline:none;background:white;font-weight:600">
    ...
    <div style="display:grid;grid-template-columns:repeat(3,1fr);gap:16px;margin-bottom:24px">
        @foreach([
            ['Total Votes Cast', number_format($totalVotes), '#e11d74', 'M9 12l2 2 4-4M7.835 4.697a3.42 3.42 0 001.946-.806 3.42 3.42 0 014.438 0 3.42 3.42 0 001.946.806 3.42 3.42 0 013.138 3.138 3.42 3.42 0 00.806 1.946 3.42 3.42 0 010 4.438 3.42 3.42 0 00-.806 1.946 3.42 3.42 0 01-3.138 3.138 3.42 3.42 0 00-1.946.806 3.42 3.42 0 01-4.438 0 3.42 3.42 0 00-1.946-.806 3.42 3.42 0 01-3.138-3.138 3.42 3.42 0 00-.806-1.946 3.42 3.42 0 010-4.438 3.42 3.42 0 00.806-1.946 3.42 3.42 0 013.138-3.138z'],
            ...
            ['Categories',       $selectedEvent->categories->count(), '#6f4497', 'M19 11H5m14 0a2 2 0 012 2v6a2 2 0 01-2 2H5a2 2 0 01-2-2v-6a2 2 0 012-2m14 0V9a2 2 0 00-2-2M5 11V9a2 2 0 012-2m0 0V5a2 2 0 012-2h6a2 2 0 012 2v2M7 7h10'],
        ...
        <div style="background:white;border:1px solid #e5e7eb;border-radius:16px;padding:20px;display:flex;align-items:center;gap:16px">
            ...
            <div>
                ...
                <p style="font-size:22px;font-weight:800;color:#241038">{{ $val }}</p>
            </div>
        </div>
        @endforeach
    </div>
    ...
    <div style="background:white;border:1px solid #e5e7eb;border-radius:16px;padding:24px;margin-bottom:20px">
        <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:20px">
            <div>
                <h2 style="font-size:16px;font-weight:800;color:#241038">{{ $result['category']->name }}</h2>
            ...
            <a href="{{ route('admin.events.results', $selectedEvent) }}"
               style="font-size:13px;font-weight:600;color:#e11d74;text-decoration:none">Full results →</a>
        </div>
        ...
        <div style="margin-bottom:14px">
            <div style="display:flex;align-items:center;gap:12px;margin-bottom:6px">
                <div style="width:26px;height:26px;border-radius:8px;display:flex;align-items:center;justify-content:center;font-size:11px;font-weight:800;flex-shrink:0;{{ $isFirst ? 'background:#e11d74;color:white' : 'background:#f3f4f6;color:#9ca3af' }}">
                    #{{ $rank }}
                </div>
                <p style="flex:1;font-weight:600;color:#241038;font-size:14px">{{ $nom->name }}</p>
                <p style="font-weight:700;color:#241038;font-size:13.5px">{{ number_format($nom->vote_total) }}</p>
                <p style="font-weight:700;color:#e11d74;font-size:13.5px;min-width:44px;text-align:right">{{ $pct }}%</p>
            </div>
            <div style="height:8px;background:#f3f4f6;border-radius:4px;overflow:hidden">
                <div style="height:100%;border-radius:4px;background:{{ $isFirst ? 'linear-gradient(90deg,#e11d74,#6f4497)' : '#e5e7eb' }};width:{{ $pct }}%;transition:width .5s"></div>
            </div>
        </div>
        @empty
    ...
    <div style="background:white;border:1px solid #e5e7eb;border-radius:16px;padding:56px;text-align:center">
        ...
        <a href="{{ route('admin.events.show', $selectedEvent) }}" style="display:inline-block;margin-top:12px;color:#e11d74;font-weight:600;font-size:13.5px;text-decoration:none">Manage event →</a>
    </div>
    @endforelse
    ...
    <div style="background:white;border:1px solid #e5e7eb;border-radius:16px;padding:56px;text-align:center">
        ...
        <a href="{{ route('admin.events.create') }}" style="display:inline-block;margin-top:12px;color:#e11d74;font-weight:600;text-decoration:none">Create your first event →</a>
    </div>
    @endif
</div>
