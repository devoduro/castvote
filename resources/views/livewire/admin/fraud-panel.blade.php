<div>
    <div style="margin-bottom:24px">
        <h2 style="font-size:20px;font-weight:800;color:#1a0030;margin-bottom:4px">Fraud Signals — {{ $event->name }}</h2>
    ...
    <div style="display:grid;grid-template-columns:repeat(4,1fr);gap:14px;margin-bottom:24px">
        ...
        <div style="background:white;border:1px solid #e5e7eb;border-radius:16px;padding:18px;position:relative;overflow:hidden">
            <div style="position:absolute;top:0;left:0;right:0;height:3px;background:linear-gradient(90deg,#e91e8c,#7c3aed)"></div>
            ...
            <p style="font-size:26px;font-weight:900;color:#1a0030">{{ number_format($ch->total) }}</p>
        ...
        <table style="width:100%;border-collapse:collapse">
            ...
            <tbody>
                ...
                <tr style="border-bottom:1px solid #fef2f2">
                    <td style="padding:10px 0;font-family:monospace;font-size:13.5px;color:#1a0030">{{ $v->voter_phone }}</td>
    ...
    <div style="background:white;border:1px solid #e5e7eb;border-radius:16px;overflow:hidden;margin-bottom:24px">
        <div style="padding:16px 20px;border-bottom:1px solid #f3f4f6">
            <h3 style="font-weight:700;color:#1a0030;font-size:14px">High Volume Voters <span style="color:#9ca3af;font-weight:400;font-size:13px">(top 50, &gt; 200 votes)</span></h3>
        </div>
        ...
        <table style="width:100%;border-collapse:collapse">
            ...
            <tbody>
                ...
                <tr style="border-bottom:1px solid #f9fafb" onmouseover="this.style.background='#fafafa'" onmouseout="this.style.background=''">
                    <td style="padding:12px 20px;font-family:monospace;font-size:13.5px;color:#1a0030;font-weight:600">{{ $v->voter_phone }}</td>
                    <td style="padding:12px 20px;text-align:right;font-family:monospace;font-weight:800;color:#e91e8c;font-size:15px">{{ number_format($v->total_votes) }}</td>
    ...
    <div style="background:white;border:1px solid #e5e7eb;border-radius:16px;padding:20px">
        <h3 style="font-weight:700;color:#1a0030;font-size:14px;margin-bottom:16px">Hourly Vote Trend <span style="color:#9ca3af;font-weight:400;font-size:13px">(last 48 hours)</span></h3>
        ...
        <div style="display:flex;align-items:flex-end;gap:3px;height:80px">
            ...
            <div style="flex:1;display:flex;flex-direction:column;align-items:center;gap:2px" title="{{ $h->hour }}: {{ number_format($h->votes) }} votes">
                <div style="width:100%;border-radius:4px 4px 0 0;background:linear-gradient(180deg,#e91e8c,#c2185b);min-height:2px;height:{{ round($h->votes / $maxV * 72) }}px;transition:height .3s"></div>
            </div>
            @endforeach
        </div>
