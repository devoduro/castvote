<div>
    <div style="display:flex;align-items:flex-start;justify-content:space-between;gap:16px;margin-bottom:20px">
        <div>
            <h3 style="font-size:17px;font-weight:800;color:#241038">{{ $category->name }} — Nominees</h3>
            <p style="color:#9ca3af;font-size:13px;margin-top:2px">
                Category code: <span style="font-family:monospace;font-weight:700;color:#6f4497">{{ $category->code }}</span>
            </p>
        </div>
        <div style="display:flex;gap:8px">
            ...
            <button wire:click="openCreate"
                    style="display:inline-flex;align-items:center;gap:6px;background:linear-gradient(135deg,#3c1f56,#4a2769);color:white;border:none;border-radius:10px;padding:10px 18px;font-size:13px;font-weight:700;cursor:pointer;box-shadow:0 4px 14px rgba(45,0,80,.25)">
    ...
    {{-- Nominee form --}}
    @if($showForm)
    <div style="background:#fff1f7;border:1.5px solid #fecce0;border-radius:14px;padding:20px;margin-bottom:20px">
        <h4 style="font-weight:700;color:#241038;font-size:14px;margin-bottom:14px">{{ $editingId ? 'Edit Nominee' : 'New Nominee' }}</h4>
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:12px;margin-bottom:12px">
            <div>
                ...
                <input wire:model="name" type="text" placeholder="Sarkodie"
                       style="width:100%;border:1.5px solid #e5e7eb;border-radius:10px;padding:9px 12px;font-size:13.5px;outline:none;color:#241038;background:white"
            ...
            <div>
                ...
                <input wire:model="code" type="text" placeholder="01" maxlength="10"
                       style="width:100%;border:1.5px solid #e5e7eb;border-radius:10px;padding:9px 12px;font-size:13.5px;font-family:monospace;outline:none;color:#241038;background:white"
                       onfocus="this.style.borderColor='#e11d74'" onblur="this.style.borderColor='#e5e7eb'">
        ...
        <div style="margin-bottom:12px">
            ...
            <textarea wire:model="bio" rows="2" placeholder="Short bio…"
                      style="width:100%;border:1.5px solid #e5e7eb;border-radius:10px;padding:9px 12px;font-size:13.5px;outline:none;color:#241038;background:white;resize:vertical"
                      onfocus="this.style.borderColor='#e11d74'" onblur="this.style.borderColor='#e5e7eb'"></textarea>
        </div>
        ...
        <div style="display:flex;gap:8px">
            <button wire:click="save" wire:loading.attr="disabled"
                    style="background:linear-gradient(135deg,#e11d74,#a00e53);color:white;border:none;border-radius:10px;padding:9px 20px;font-size:13px;font-weight:700;cursor:pointer">
            ...
            <tbody>
                ...
                <tr style="border-bottom:1px solid #f9fafb" onmouseover="this.style.background='#fafafa'" onmouseout="this.style.background=''">
                    <td style="padding:14px 20px">
                        <span style="background:#f7f5fb;color:#6f4497;padding:3px 10px;border-radius:8px;font-family:monospace;font-size:13px;font-weight:800">{{ $nom->code }}</span>
                    </td>
                    <td style="padding:14px 20px">
                        <div style="display:flex;align-items:center;gap:10px">
                            ...
                            @else
                            <div style="width:34px;height:34px;border-radius:8px;background:linear-gradient(135deg,#e11d7420,#6f449720);display:flex;align-items:center;justify-content:center;flex-shrink:0;font-size:12px;font-weight:700;color:#6f4497">
                                {{ strtoupper(substr($nom->name,0,2)) }}
                            </div>
                            @endif
                            <span style="font-weight:600;color:#241038;font-size:14px">{{ $nom->name }}</span>
                        </div>
                    </td>
