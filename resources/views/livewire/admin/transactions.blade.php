<div>
    <div style="margin-bottom:24px">
        <h1 style="font-size:22px;font-weight:800;color:#1a0030;margin-bottom:4px">My Event Transactions</h1>
    ...
    <div style="display:grid;grid-template-columns:repeat(4,1fr);gap:16px;margin-bottom:24px">
        @foreach([
            ['Gross Revenue',     'GHS '.number_format($grossRevenue/100,2), 'Total Collected',  '#e91e8c', 'M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8c1.11 0 2.08.402 2.599 1M12 8V7m0 1v8m0 0v1m0-1c-1.11 0-2.08-.402-2.599-1M21 12a9 9 0 11-18 0 9 9 0 0118 0z'],
            ['Net Earnings',      'GHS '.number_format($grossRevenue*0.95/100,2), 'After Commission', '#059669', 'M13 7h8m0 0v8m0-8l-8 8-4-4-6 6'],
            ['Total Engagement',  number_format($totalVotes),  'Total Votes Cast',  '#7c3aed', 'M9 12l2 2 4-4M7.835 4.697a3.42 3.42 0 001.946-.806 3.42 3.42 0 014.438 0 3.42 3.42 0 001.946.806 3.42 3.42 0 013.138 3.138 3.42 3.42 0 00.806 1.946 3.42 3.42 0 010 4.438 3.42 3.42 0 00-.806 1.946 3.42 3.42 0 01-3.138 3.138 3.42 3.42 0 00-1.946.806 3.42 3.42 0 01-4.438 0 3.42 3.42 0 00-1.946-.806 3.42 3.42 0 01-3.138-3.138 3.42 3.42 0 00-.806-1.946 3.42 3.42 0 010-4.438 3.42 3.42 0 00.806-1.946 3.42 3.42 0 013.138-3.138z'],
        ...
        <div style="background:white;border:1px solid #e5e7eb;border-radius:16px;padding:20px">
            ...
            <p style="font-size:22px;font-weight:800;color:#1a0030;margin-bottom:4px">{{ $val }}</p>
    ...
    <div style="background:white;border:1px solid #e5e7eb;border-radius:16px;overflow:hidden">
        <div style="padding:16px 20px;border-bottom:1px solid #f3f4f6;display:flex;align-items:center;gap:12px;flex-wrap:wrap">
            ...
            <select wire:model.live="eventId"
                    style="border:1.5px solid #e5e7eb;border-radius:10px;padding:9px 14px;font-size:13.5px;color:#1a0030;outline:none;background:white">
            ...
            <tbody>
                ...
                <tr style="border-bottom:1px solid #f9fafb;transition:background .1s" onmouseover="this.style.background='#fafafa'" onmouseout="this.style.background=''">
                    <td style="padding:13px 16px">
                        <p style="font-family:monospace;font-size:12px;color:#1a0030;font-weight:600">{{ Str::limit($payment->provider_reference, 18) }}</p>
                        <p style="font-size:11px;color:#9ca3af">{{ $payment->event->name }}</p>
                    </td>
                    <td style="padding:13px 16px;font-size:13px;color:#1a0030">—</td>
                    <td style="padding:13px 16px;font-size:13px;color:#1a0030">{{ $payment->phone_number }}</td>
                    <td style="padding:13px 16px">
                        <span style="background:#f3f0ff;color:#7c3aed;padding:3px 10px;border-radius:20px;font-size:11px;font-weight:700;text-transform:uppercase">{{ $channel }}</span>
                    </td>
                    <td style="padding:13px 16px;font-weight:700;color:#1a0030;font-size:14px">{{ $payment->amountInGhs() }}</td>
